...
import io.github.lcenhancer.base.annotation.Requires;
 ...
 * @author Jidcoo
 * @see Require
 * @see io.github.lcenhancer.base.interfaces.InputProvider
 * @see io.github.lcenhancer.base.interfaces.OutputConsumer
 * @since 1.0.0
 */
abstract class AbsIORequireSupporter<IO_SOURCE> {
    ...
    protected List<Require> awareAcceptableIOSourceRequire(LeetcodeJavaDebugEnhancer leetcodeJavaDebugEnhancer) {
        ...
        requireList = requireList.stream().filter(require -> {
            ...
            for (Class<?> type : types) {
                if (!ACCEPTABLE_IO_SOURCE_TYPES.containsKey(type)) {
                    LogUtil.logW("AbsIORequireSupporter: unaccepted io source type in IO-Require-Annotation(types are %s, values are" +
                                    " %s): %s",
            ...
            if (typesLength > 1 && typesLength > valuesLength) {
                LogUtil.logW("AbsIORequireSupporter: invalid types-length(%d) and values-length(%d) in IO-Require-Annotation(types " +
                                "are %s, values are %s): " +
    ...
    protected final List<IO_SOURCE> awareIOSources(LeetcodeJavaDebugEnhancer leetcodeJavaDebugEnhancer) {
        ...
        if (ContainerUtil.isEmpty(requireList)) {
            return ioSourceList;
        }
