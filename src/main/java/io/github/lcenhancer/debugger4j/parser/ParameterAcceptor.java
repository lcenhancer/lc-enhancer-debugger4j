...
final class ParameterAcceptor extends BaseParameterAcceptStrategy<BaseParameterAcceptStrategy.ParameterAcceptResult> {
     ...
     */
    private static final String BUILT_IN_PARAMETER_ACCEPTANCE_STRATEGY_PACKAGE = "io.github.lcenhancer.debugger4j.parser.builtin";
    ...
    ParameterAcceptor() {
        ...
        List<BaseParameterAcceptStrategy> strategies = BeanUtil.collectBeans(BaseParameterAcceptStrategy.class,
                ...
                (Class<BaseParameterAcceptStrategy> type) ->
                        type.isAnnotationPresent(Require.class) &&
                        ReflectUtil.isExtendsClass(type, BaseParameterAcceptStrategy.class) &&
                        !Modifier.isAbstract(type.getModifiers()),
                (Class<BaseParameterAcceptStrategy> beanType) -> ReflectUtil.createInstance(beanType))
     ...
     *
     * @param customStrategies the custom strategies.
     * @return null if the custom strategies list is empty,
     * else a combination parameter accepting strategy set.
     */
    Map<Class<?>, Set<BaseParameterAcceptStrategy<?>>> combineCustomStrategies(List<BaseParameterAcceptStrategy<?>> customStrategies) {
        if (ContainerUtil.isEmpty(customStrategies)) {
            return null;
        }
        ...
        customStrategies.forEach(strategy -> addParameterAcceptStrategy(strategy.getAcceptableType(), strategy, combinedCustomStrategiesMap));
     ...
     */
    private void addParameterAcceptStrategy(Class<?> type,
                                            BaseParameterAcceptStrategy<?> strategy,
