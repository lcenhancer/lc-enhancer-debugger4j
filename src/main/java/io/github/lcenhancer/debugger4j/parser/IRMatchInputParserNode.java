...
import io.github.lcenhancer.base.utils.OrderUtil;
...
final class IRMatchInputParserNode extends InputParserNode {
    ...
    Object parse(InputParserContext context) {
        ...
        Map<LeetcodeInvoker, Map<Integer, Stack<ParameterAcceptStrategyExceptionTracer>>> matchTracer = new HashMap<>();

        // Try to combine custom strategies if the availableParameterAcceptStrategies are not empty.
        Map<Class<?>, Set<BaseParameterAcceptStrategy<?>>> strategies = parameterAcceptor.combineCustomStrategies(context.getAvailableParameterAcceptStrategies());
        ...
        if (ContainerUtil.isNotEmpty(leetcodeInvokers)) {
            ...
            for (LeetcodeInvoker leetcodeInvoker : leetcodeInvokers) {
                ...
                Map<Integer, Stack<ParameterAcceptStrategyExceptionTracer>> invokerMatchTracerMap = new HashMap<>();
                ...
                for (int i = 0; i < parameterTypes.length; i++) {
                    ...
                    BaseParameterAcceptStrategy.ParameterAcceptResult result = parameterAcceptor.acceptParameter(
                            copiedObject,
    ...
    private void logDetailAfterIRMatching(LeetcodeInvoker bossInvoker, List<LeetcodeInvoker> invokers,
                                          List<Object> input,
                                          Map<LeetcodeInvoker, Map<Integer, Stack<ParameterAcceptStrategyExceptionTracer>>> matchTracer) {
        if (Objects.isNull(bossInvoker)) {
            ...
            logBuffer.append("LeetcodeInvokers: ").append(invokers.size()).append(",  IRInputs: ").append(input.size());
            logBuffer.append("\n<TracersDetail>\n");
            int invokerIdx = 0;
            for (Map.Entry<LeetcodeInvoker, Map<Integer, Stack<ParameterAcceptStrategyExceptionTracer>>> methodListEntry :
                    matchTracer.entrySet()) {
                logBuffer.append("LeetcodeInvoker-").append(invokerIdx++);
                logBuffer.append(": ");
                logBuffer.append(methodListEntry.getKey().toGenericString());
                logBuffer.append("\n<Tracers(").append(methodListEntry.getValue().size()).append(")>\n");
                Map<Integer, Stack<ParameterAcceptStrategyExceptionTracer>> stackMap = methodListEntry.getValue();
                for (Map.Entry<Integer, Stack<ParameterAcceptStrategyExceptionTracer>> stackEntry : stackMap.entrySet()) {
                    logBuffer.append(" - Index: ").append(stackEntry.getKey());
                    logBuffer.append(",   Parameter: ").append(methodListEntry.getKey().getParameters()[stackEntry.getKey()].getName());
                    logBuffer.append(",   Type: ").append(methodListEntry.getKey().getParameters()[stackEntry.getKey()].getParameterizedType().getTypeName());
                    logBuffer.append(",   Input(").append(input.get(stackEntry.getKey()) == null ? "Null" :
                            input.get(stackEntry.getKey()).getClass().getSimpleName()).append("): ").append(GsonUtil.toJson(input.get(stackEntry.getKey())));
                    logBuffer.append("\n");
                    Stack<ParameterAcceptStrategyExceptionTracer> tracerStack = stackEntry.getValue();
                    while (!tracerStack.empty()) {
                        ParameterAcceptStrategyExceptionTracer tracer = tracerStack.pop();
            ...
            LogUtil.logE("Cannot match any leetcode invoker for IR-Input: %s\n\n%s", input,
                    logBuffer.toString());
